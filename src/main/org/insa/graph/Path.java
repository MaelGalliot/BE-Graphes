...
public class Path {
            	...
            	{        			
            		for(Arc arc : nodes.get(i)) /* Pour les arcs du noeud i */
            		{
            			if(arc.getDestination().compareTo(nodes.get(i+1))==0)/*On regarde les arcs du noeud i qui vont au noeud n+1 */
    	               	{
    	            		if(!initarc) /*Si c'est la premiËre itÈration*/
    	            		{
    	            			arcShortest = arc;//On prend un arc par dÈfaut
    	            			initarc = true;
    	            		}
    	            		else
            		...
            		if(!initarc)/*Si initarc n'a pas Ètait mis ‡ true aucun arc ne va au noeud i+1*/
    ...
    public boolean isValid() {
        ...
        if (this.isEmpty()) {
        	...
        	if (this.size() == 1) {
        		...
        		//Si l'origine du chemin est la m√™me que l'origine du premier arc
        		if (this.getOrigin().compareTo(arcs.get(0).getOrigin()) == 0) {
        			//On consid√®re momentan√©ment le chemin comme valide
        			valid = true;
        			//Mais si un seul de ses arcs a une destination qui diff√®re de celle du suivant,
    ...
    public float getLength() {
        ...
        float pathLength = 0;
        //Pour chaque arc du chemin, on r√©cup√®re sa longueur et on fait la somme
        for (Arc arc : arcs) {
    ...
    public double getTravelTime(double speed) {
    	//On convertit la vitesse donn√©e en en m/s 
    ...
    public double getMinimumTravelTime() {
    	//La dur√©e minimale
    	double travelTime = 0;
    	//Pour chaque arc, on cherche sa dur√©e minimale et on fait la somme totale
    	for (Arc arc : arcs) {
